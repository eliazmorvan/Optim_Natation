...
def _test_consistancy():
    # Should pass
    S, T, R = func.import_data_fake(20,10,4)
    func.check_consistancy(S, T, R)
    
    try :
        S, _, R = func.import_data_fake(20,10,4)
        _, T, _ = func.import_data_fake(10,10,4)
        func.check_consistancy(S, T, R)
    except :
        print("n check passed")
        
    try :
        S, T, R = func.import_data_fake(20,8,4)
        func.check_consistancy(S, T, R)
    except :
        print("m check passed")
        
    try :
        S, T, R = func.import_data_fake(20,10,3)
        func.check_consistancy(S, T, R)
    except :
